...
#ifdef OS_WINDOWS            
#ifdef UNICODE
            int chrErrLen = MAX_PATH;
			wchar_t* err_str = gai_strerror(error);
			int len = WideCharToMultiByte(CP_ACP, 0, err_str, wcslen(err_str), NULL, 0, NULL, NULL);			
			WideCharToMultiByte(CP_ACP, 0, err_str, wcslen(err_str), chrErr, len, NULL, NULL);
            chrErr[len] = 0;
            err_string = chrErr;
#else
...
Json mysql_concat_json_res(WFMySQLTask *mysql_task)
{
    ...
    {
        json["code"] = mysql_task->get_resp()->get_error_code();
        json["msg"] = mysql_task->get_resp()->get_error_msg();
    }
...
Json redis_json_res(WFRedisTask *redis_task)
{
    ...
    case WFT_STATE_SYS_ERROR:
        js["msg"] = "system error: " + std::string(strerror(error));
        break;
    ...
    case WFT_STATE_SSL_ERROR:
        js["msg"] = "SSL error: " + std::to_string(error);
        break;
    case WFT_STATE_TASK_ERROR:
        js["msg"] = "Task error: " + std::to_string(error);
        break;
    case WFT_STATE_SUCCESS:
        redis_resp->get_result(val);
        if (val.is_error())
        {
            js["msg"] = "Error reply. Need a password?\n";
    ...
    if(state == WFT_STATE_SUCCESS && cmd == "GET")
    {
        ...
        {
            js["msg"] = "value is not a string value";
        }
    }
    return js;
}
...
{   
    ...
    for(const auto &file : file_list) 
    {
        ...
        {
            XLOG_ERROR("[Error] Not a File : {:s}", file.second.c_str());
            continue;
        }
        size_t file_size;
        ...
        if (ret != StatusOK)
        {
            XLOG_ERROR( "[Error] Invalid File : {:s}", file.second.c_str());
            continue;
        }
